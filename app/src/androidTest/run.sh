...
#通过-e 添加一个键为regenerateTestsuite,值为$regeneratTestsuite  ,标识是否重新组织测试用例集合
#第一次regeneratTestsuite的值为false，如果crash发生了，就需要把这个值赋值为true，下次循环时这个值就是true了。需要组织测试用例
    adb -s $1 shell am instrument -w -e reportDir $junitReportPath  -e reportFile junit-report_${crashCount}.xml  -e regeneratTestsuite $regeneratTestsuite com.example.todolist.tests/com.example.todolist.test.runners.Runner
...
#同时把crash.txt文件pull到workplace之下
    ...
    adb -s $1 pill $testautoRootPath/crash.txt    $WORKSPACE
#现在判断workspace地下是否有没有crash.txt文件
#有的话，说明发生了crash，需要把regeneratTestsuite的值赋值为true，重新组织用例，排除已经运行的用例
